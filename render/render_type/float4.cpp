...
float float4::mag() {return sqrt(pow(x, 2) + pow(y, 2) + pow(z, 2) + pow(w, 2));}
...
// Absolute value unit vector of this vector
float4 float4::aunit()
{
	// Initialization
	static float magnitude = float4::mag();	
	...
	if(magnitude != 0)
	{
		answer.x = abs(x / magnitude);
		answer.y = abs(y / magnitude);
		answer.z = abs(z / magnitude);
		answer.w = abs(w / magnitude);
	}
